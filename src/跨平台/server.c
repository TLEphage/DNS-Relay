...
void init_socket(int port) {
    // 初始化网络（Windows需要WSAStartup，Linux不需要）
    if (init_network() != 0) {
    ...
    // 端口复用
    if (set_socket_reuse(client_socket) != 0) {
        printf("Warning: Failed to set socket reuse\n");
    }

    if (bind_socket(client_socket, &client_address) != 0) {
        printf("ERROR: Could not bind socket\n");
        close_socket_safe(client_socket);
        close_socket_safe(server_socket);
        cleanup_network();
        exit(-1);
    }
...
void poll() {
    ...
    if (set_socket_nonblocking(server_socket) != 0 ||
        set_socket_nonblocking(client_socket) != 0) {
        printf("Failed to set sockets to non-blocking mode\n");
        close_socket_safe(server_socket);
        close_socket_safe(client_socket);
        cleanup_network();
        return;
    ...
    while (1)
    {
        ...
        fds[0].events = POLL_IN;  // POLL_IN 表示可读
        fds[1].fd = server_socket;
        fds[1].events = POLL_IN;

        // 调用跨平台poll函数进行等待
        int ret = poll_sockets(fds, 2, 5); // 5ms超时

        if (ret < 0)
        {
            printf("ERROR poll: %s\n", get_error_string(GET_SOCKET_ERROR()));
            LOG_INFO("ERROR poll: %s\n", get_error_string(GET_SOCKET_ERROR()));
        }
        else if (ret > 0)
        {
            if (fds[0].revents & POLL_IN) //revents是真实发生的事件类型，与POLL_IN进行按位与运算可以判断是否有数据可读
            {
...
void receiveClient() { 
    ...
    if (query_res != NULL) {
        ...
        if(sendto(server_socket, buffer, recv_len, 0, (struct sockaddr *)&server_address, sizeof(server_address)) == SOCKET_ERROR_VALUE) {
            printf("Error sending request to remote DNS server: %s\n", get_error_string(GET_SOCKET_ERROR()));
            LOG_ERROR("Sendto failed: %s", get_error_string(GET_SOCKET_ERROR()));
